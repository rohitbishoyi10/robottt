*** Settings ***
Documentation   Test cases to Get unAssign Domain
...
*** Test Cases ***
Test Case 1 : Get UnAssign Domain
    [Documentation] 	This test is to Get UnAssign Domain
    [Tags]              valid
        ${Result}=    Post Request Of Api With Body    ${protocol}		${api_environment}		${sfcore_api_endpoint}    ${unAssign_Domain_request_body}   ${global_auth_headers}
        Validate Json Response For An API   ${Result}    product_api/get_unassign_domain.json
        Validate Response Of The Api        ${Result}   ${SUCCESS_CODE}

Test Case 2 : Get UnAssign Domain With Invalid Data
    [Documentation] 	This test is to Get UnAssign Domain with invalid data
    [Tags]              invalid
